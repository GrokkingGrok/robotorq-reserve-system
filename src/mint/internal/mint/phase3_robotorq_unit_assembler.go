...
func (a *Phase3RoboTorqUnitAssembler) Start(ctx context.Context) {
	...
	for {
		// Immediate cancellation check before starting any work
		if ctx.Err() != nil {
			a.logger.Info("phase3 assembler shutting down", "reason", ctx.Err())
			return
		}

		// Build Level 2 merkle tree (blocks until 1000 ingots ready or ctx cancelled)
		...
		// Cancellation check after publish to avoid racing next build
		if ctx.Err() != nil {
			a.logger.Info("phase3 assembler stopping after cancellation post publish")
			return
		}
	}
}
...
func (a *Phase3RoboTorqUnitAssembler) assembleUnit(merkleResult *Level2MerkleResult) (*models.Phase3RoboTorqUnit, error) {
	...
	a.logger.Info("phase3 unit metadata",
		...
		"tree_height", merkleResult.TreeHeight,
		"note", "metadata logged for observability, not persisted to unit (digger_ids removed)")
